...
flags.DEFINE_integer('batch_size', 256, 'Size of batches per epoch.') #4096
...
text = []
with open('data/' + FLAGS.dataset + '_test.txt', 'r') as f:
    for line in f.readlines():
        text.append(line.strip())
with open('data/' + FLAGS.dataset + '_result.txt', 'w') as f:
    f.write('text\tlabels\tpred\n')
    for i in range(len(text)):
        f.write(text[i]+'\t'+str(labels[i])+'\t'+str(pred[i])+'\n')
'''
# For visualization
doc_vectors = []
