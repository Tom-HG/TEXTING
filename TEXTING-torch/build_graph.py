...
# print(shuffle_doc_name_list[:5]) #['3374\ttrain\t1'] id\train or test\label
# print('shuffle_doc_words_list') #['ryan gosling is , in a word , brilliant as the conflicted daniel'] text
# print(shuffle_doc_words_list[:5])
...
def build_graph(start, end):
    ...
    for i in tqdm(range(start, end)):
        ...
        text.append(shuffle_doc_words_list[i])
        if truncate:
        ...
        # sliding windows
        # print(doc_words)
        # print(doc_words[0:doc_len//3])
        # print(doc_words[doc_len//3 : doc_len//3*2])
        # print(doc_words[doc_len//3*2 : doc_len])
        windows_1 = []
        windows_2 = []
        windows_3 = []
        if doc_len <= window_size:
            windows_1.append(doc_words[0:doc_len//3])
            windows_2.append(doc_words[doc_len//3 : doc_len//3*2])
            windows_3.append(doc_words[doc_len//3*2 : doc_len])
        else:
            for j in range(doc_len - window_size + 1):
                window1 = doc_words[j: j + window_size//3]
                windows_1.append(window1)
                window2 = doc_words[j + window_size//3 : j + window_size//3*2]
                windows_2.append(window2)
                window3 = doc_words[j + window_size//3*2 : j + window_size]
                windows_3.append(window3)

        windows = [windows_1, windows_2, windows_3]
        # print(windows)
        for j in range(3):
            word_pair_count = {}
            for window in windows[j]:
    ...
    return x_adj, x_feature, y, doc_len_list, vocab_set, text

def build_adj(word_pair_count,doc_word_id_map,doc_nodes):
    row = []
    col = []
    weight = []
    for key in word_pair_count:
...
def build_graph_v2(start, end):
    ...
    for i in tqdm(range(start, end)):
        ...
        #distant
        window_1 = window_size //3 * 1 //2
        window_2 = window_size //3 * 2 //2
        window_3 = window_size //3 * 3 //2
        # print(window_1)
        # print(window_2)
        # print(window_3)

        for p in range(0, doc_len):
            word_p = doc_words[p]
            word_p_id = word_id_map[word_p]
            #windows size
            if p <= window_size // 2:
                #left
                l = 0
                r = p+window_size//2 + mod
            elif p + window_size // 2 + mod >= doc_len:
                #right
                l = p - window_size // 2
                r = doc_len
                for q in range(l, r):
            ...
            for q in range(l, r):
                ...
                # word co-occurrences as weights
                if p == q:
                    if word_pair_key in word_pair_count[0]:
                        word_pair_count[0][word_pair_key] += 1.
                    else:
                        word_pair_count[0][word_pair_key] = 1.
                    if word_pair_key in word_pair_count[1]:
                        word_pair_count[1][word_pair_key] += 1.
                    else:
                        word_pair_count[1][word_pair_key] = 1.
                    if word_pair_key in word_pair_count[2]:
                        word_pair_count[2][word_pair_key] += 1.
                    else:
                        word_pair_count[2][word_pair_key] = 1.
                    continue

                if word_pair_key in word_pair_count[m]:
                    word_pair_count[m][word_pair_key] += 1.
                else:
                    word_pair_count[m][word_pair_key] = 1.

        # print(word_pair_count[0])
        # print(word_pair_count[1])
        # print(word_pair_count[2])
            # bi-direction
            # word_pair_key = (word_q_id, word_p_id)
            # if word_pair_key in word_pair_count:
            #     word_pair_count[word_pair_key] += 1.
        ...
        for x in range(3):
            x_adj[x].append(build_adj(word_pair_count[x],doc_word_id_map,doc_nodes))
...
# build_graph_v2(start=0, end=1)


print('building graphs for training')
x_adj, x_feature, y, _, _, _ = build_graph_v2(start=0, end=real_train_size)
print('building graphs for training + validation')
allx_adj, allx_feature, ally, doc_len_list_train, vocab_train, _ = build_graph_v2(start=0, end=train_size)
print('building graphs for test')
tx_adj, tx_feature, ty, doc_len_list_test, vocab_test, test_text = build_graph_v2(start=train_size, end=train_size+test_size)
doc_len_list = doc_len_list_train + doc_len_list_test
print(np.shape(x_adj))
print(np.shape(x_feature))
# print(x_feature[0][0][0][:5])
# print('-----')
# print(x_feature[1][0][0][:5])
# print('-----')
# print(x_feature[2][0][0][:5])
# print('-----')
# print(x_adj[0][0][0])
# print('------')
# print(x_adj[1][0][0])
...
with open("data/{}_test.txt".format(dataset),'w') as f:
